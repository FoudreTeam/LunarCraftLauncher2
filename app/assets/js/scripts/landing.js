...
const refreshServerStatus = async (fade = false) => {
    ...
    try {
        ...
        loggerLanding.warn('Impossible d"actualiser le statut du serveur.')
...
async function asyncSystemScan(effectiveJavaOptions, launchAfter = true){
    ...
    setLaunchDetails('Vérification info système..')
    toggleLaunchArea(true)
    setLaunchPercentage(0, 100)
    ...
    if(jvmDetails == null) {
        ...
        setOverlayHandler(() => {
            ...
            try {
                ...
                loggerLanding.error('Erreur Java', err)
        ...
        setDismissHandler(() => {
            $('#overlayContent').fadeOut(250, () => {
                ...
                setOverlayContent(
                    'Java requis<br>pour le lancement du jeu',
                    `Une installation valide de Java x64 ${effectiveJavaOptions.suggestedMajor} est requis pour lancer le jeu.<br><br>Merci de vous référer au <a href="https://github.com/dscalzi/HeliosLauncher/wiki/Java-Management#manually-installing-a-valid-version-of-java">Java Management Guide</a> Pour des instructions sur l'installation manuelle de Java.`,
                    'Je comprend',
                    'Retour'
                )
                setOverlayHandler(() => {
...
async function downloadJava(effectiveJavaOptions, launchAfter = true) {
    ...
    const eLStr = 'Exctration de Java...'
    let dotStr = ''
...
async function dlAsync(login = true) {
    ...
    fullRepairModule.childProcess.on('error', (err) => {
        loggerLaunchSuite.error('Erreur de lancement.', err)
        showLaunchFailure('Erreur pendant le lancement du jeu.', err.message || 'Voir la console (CTRL + Shift + i) pour plus de détails.')
    })
    ...
    loggerLaunchSuite.info('Vérification des fichiers.')
